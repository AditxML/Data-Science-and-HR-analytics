...
author: "Sacha Kapoor"
date: "11 November 2024 (updated: `r Sys.Date()`)"
output:
...
classification and prediction exercises often look quite similar. The naming convention comes from the different types of output or dependent variables. If the aim is to predict quantitative outputs, people use the term regression. IF the aim is to predict qualitative outputs, then people use the term classification. These prediction tasks have a lot in common. Importantly, they both involve function approximation. Ordered categorical variables fall somewhere in between these two.     
...
data.frame(fgl$type[test],nearest1,nearest5,nearest10,nearest20)
```
...
```{r, echo=FALSE}
#### ******* German Credit Data ******* ####
credit <- read.csv("C:/Users/aditc/OneDrive/Documents/credit.csv")
...
library(pROC)  # Load pROC package instead of sourcing roc.R

png(file="ROCCurve.png", width=600, height=350)
par(mai=c(.9,.9,.2,.1), mfrow=c(1,2))

# in-sample ROC curve
roc_curve <- roc(default, pred)
plot(roc_curve, main="in-sample", bty="n")
...
roc_curve_oos <- roc(defaultoos, predoos)
plot(roc_curve_oos, main="out-of-sample", bty="n")
...
xfgl <- sparse.model.matrix(type~.*Na, data=fgl)[,-1] 
gtype <- fgl$type
glassfit <- cv.glmnet(xfgl, gtype, family="multinomial") #cross validation experiments
plot(glassfit)

xfgl <- sparse.model.matrix(type~.*Ba, data=fgl)[,-1] 
gtype <- fgl$type
glassfit <- cv.glmnet(xfgl, gtype, family="multinomial") #cross validation experiments
plot(glassfit)
...
DeltaBAl <- B["Al", "WinNF"] - B["Al", "WinF"]
exp(DeltaBAl)
1 - exp(DeltaBAl)

DeltaBMgNFvsVeh <- B["Mg", "WinNF"] - B["Mg", "Veh"]
exp(DeltaBMgNFvsVeh)
1 - exp(DeltaBMgNFvsVeh)

train_idx <- sample(1:nrow(xfgl), size=0.5 * nrow(xfgl))
glassfit_subset <- cv.glmnet(xfgl[train_idx, ], gtype[train_idx], family="multinomial")
B_subset <- coef(glassfit_subset, select="min")
B_subset <- do.call(cbind, B_subset) 
colnames(B_subset) <- levels(gtype) # column names dropped in previous command. This command adds them back.
DeltaBMg_subset <- B_subset["Mg", "WinNF"] - B_subset["Mg", "WinF"]
exp(DeltaBMg_subset)
1 - exp(DeltaBMg_subset)
---
...
---
# Discriminatory vs generative classifiers

Logistic regression used a lot in natural language processing. Can use logistic regression to do supervised ML for classification. Can classify an object into 1 of 2 classes or 1 of many classes. 

Logistic regression is a discriminatory classifier. Goal is to learn how to distinguish classes. It can help you determine where a job candidate is suitable for an interview or not. 

This is in contrast to generative classifier. Here the goal is to understand what a suitable job candidate would look like. Here we would generate an ideal candidate and then determine the fit of a prospective candidate to that ideal candidate. Naive Bayes is a generative classifier. 

Logistic will assign a class $c$ to document $d$ by estimating a likelihood $P(c|d)$. A generative classifier like naive bayes would set the class by choosing $c$ to maximize $P(d|c)P(c)$. First term tells us how to generate the features of a document in the case where we know the class $c$. This makes more sense. 
