...
```{r}
oj <- read.csv("C:/Users/aditc/OneDrive/Documents/oj.csv")
head(oj, n=5)    
tail(oj, n=5)    
glm(log(sales) ~ brand + log(price), data=oj)
experimentmodel <- glm(log(price) ~ log(sales) + brand, data = oj)
...
oj$mybrand2 = relevel(oj$brand, "minute.maid")
x <- model.matrix(~ mybrand2 + log(price), data=oj); head(x)
```
...
glm(log(sales) ~ log(price)*mybrand*feat, data=oj)
glm(log(sales) ~ log(price)*mybrand2*feat, data=oj)
...
```{r}
email <- read.csv("C:/Users/aditc/OneDrive/Documents/spam.csv")
dim(email)
colnames(email) 
head(email)
tail(email)
...
coef(spammy)["word_meeting"]; exp(coef(spammy)["word_meeting"]); 1/exp(coef(spammy)["word_meeting"])
```
...
predict(spammy, newdata = email[c(1:5),], type="response")

predict(spammy, newdata = email[c(1,4000),], type="link")
...
n <- nrow(email)
email_half <- email[1:(n / 2), ]
spammy <- glm(spam ~ ., data=email_half, family='binomial')
D <- summary(spammy)$deviance  
D0 <- summary(spammy)$null.deviance
R2 <- 1 - D / D0

D; D0; R2

email_half2 <- email[(n / 2 + 1):n, ] 
spammy <- glm(spam ~ ., data=email_half2, family='binomial')
D <- summary(spammy)$deviance  
D0 <- summary(spammy)$null.deviance 
R2 <- 1 - D / D0 
...
head(email_half[,"spam"]); tail(email_half[,"spam"]); head(email_half2[,"spam"]); tail(email_half2[,"spam"])
---
