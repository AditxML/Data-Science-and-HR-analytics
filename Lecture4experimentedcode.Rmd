...
author: "Sacha Kapoor"
date: "November 2024 (updated: `r Sys.Date()`)"
output:
...
\begin{align*}
dev_{In}(\widehat{\boldsymbol\beta}_{IS}) = \sum_{i=1}^{n} (y_i - \mathbf{x}_i' \widehat{\boldsymbol\beta}_{IS})^2
\end{align*}
...
```{r}
SC <- read.csv("C:/Users/aditc/OneDrive/Documents/semiconductor.csv")
...
null <- glm(FAIL~1, data=SC)
fwd_bic <- step(null, scope=formula(full), direction="forward", k=log(nrow(SC)))
length(coef(fwd_bic))
---
...
# LASSO/Ridge Comparison ($y= \beta_0 + e$)


```{r, echo=FALSE, out.width = "90%", fig.align='center'}

knitr::include_graphics("figs/LASSO and Ridge - Intercept Regression Model.png")
...
To summarize the last figures: 
  * True $\beta_0=1$. 
  * Estimation cost minimized at $\beta=1$. 
  * Testing cost minimized at $\beta_0=0$. 
  * $\hat{\beta}_{0LASSO}$ and $\hat{\beta}_{0ridge}$ both approach 0 from left as penalty increases. 
  * LASSO criterion (estimation + testing) always kinked at 0. 
  * LASSO criterion increases sharply at 0 if penalty is sufficiently large. 
  * Ridge criterion continues to decrease. 
  * Without a kink you cannot generate a kink in the constrained criterion. 
  * You always end up away from 0, even if it is just a little bit. This is why LASSO selects 0 when ridge does not. 

Can check this with math (intercept only model). Is useful for understanding.  

---

# LASSO v. Ridge ($y= \beta_0 x_0 + \beta_1 x_1 + e$)


```{r, echo=FALSE, out.width = "90%", fig.align='center'}

knitr::include_graphics("figs/LASSO and Ridge - Multiple Regression Model.png")
...
To summarize: 
  * First model shows no corner solutions. 
  * Second model shows that as you shrink the budget, more likely to get corner solution. 
  * Third model shows that multicollinearity also generates corner solutions. 
  * An interesting aspect of last case is that it does not work if the coefficients have identical magnitudes. This makes sense because there is no way to sort between them. 

---

Let us see how LASSO plays our with the math. The LASSO optimization problem is:

$\min_{\boldsymbol\beta} \frac{1}{2} \sum_{i=1}^n (y_i - \mathbf{x}_i'\boldsymbol\beta)^2 + \lambda \sum_{i=1}^p |\beta_p|$

First term is OLS criterion. Second term is LASSO regularization penalty. $\lambda$ is Lagrange multiplier associated with LASSO penalty. It reflects the effects of the sparsity constraint on the coefficients $\boldsymbol\beta$. The Karush-Kuhn-Tucker (KKT) first order conditions associated with this optimization problem are 

$\sum_{i=1}^n x_{i1}(y_i - \mathbf{x}_i'\boldsymbol\beta) = \lambda\gamma_1$

$\cdots$

$\sum_{i=1}^n x_{ip}(y_i - \mathbf{x}_i'\boldsymbol\beta) = \lambda\gamma_p$
...
$$
  \gamma_1\in 
    \begin{cases}
      \text{sign}(\beta_1) & \text{if } \beta_1 \neq 0 \\\
      [-1, 1] & \text{if } \beta_1 = 0
    \end{cases}
$$
and $\gamma_2,...,\gamma_p$ are defined analogously.  

---
FOCs need to be defined at estimator, not true beta. Can consider simple case. Can interpret left hand side of FOC as the covariance between each xi and the residual. With Lasso we are therefore equating the covariance with the penalty (it's like equating marginal benefit with marginal cost). Contrast this with OLS where we are equating this covariance with 0. The marginal benefit is the reduction in estimation error that comes with including a non-zero coefficient. The marginal cost is the penalty or marginal testing cost. 

This implies that some covariances will be between negative and positive lambda. We will drop those ones from the estimation. For these ones the penalty is too large relative to the covariance between the covariate and the residual. interesting that marginal benefit here is the covariance between the covariate and the residual. It's like saying that the covariate has to pick up enough of the residual to make it worth it. A different type of thinking. If we try to pin down the true model, we want the covariate to pick up none of the residual variation. 


---

If goal is prediction, then we are making an implicit assumption that the true model is "sparse". True model has fewer covariates than observations. How many fewer? $< a \sqrt{N}/\max\{N,p\}$. Can plot this function in R. Where does this function come from? Theory. We do not discuss details here. But we can plot it. 


calculate_sparsity_bound <- function(p, N, alpha) {
  return(alpha * sqrt(N) / sqrt(p))}#Define a function to calculate the sparsity bound
...
plot(p_values, bound_values, type = "l", col = "blue", lwd = 2, 
     xlab = "Number of Covariates (p)", ylab = "Sparsity Bound",
     main = "Sparsity Bound vs. Number of Covariates",
     ylim = c(0, max(bound_values)))# Plot the sparsity bounds
...
web <- read.csv("C:/Users/aditc/OneDrive/Documents/browser-domains.csv")
## Read in actual website names and relabel site factor
sitenames <- scan("C:/Users/aditc/OneDrive/Documents/browser-sites.txt", what="character")
...
# what sites did household 1 visit? - print code below gives all the sites, some include atdmt.com, google.com and aol.com
print(xweb[1, xweb[1,]!=0])
## now read in the spending data 
yspend <- read.csv("C:/Users/aditc/OneDrive/Documents/browser-totalspend.csv", row.names=1)  # us 1st column as row names
...
```{r}
betamin = coef(cv.spender, select="min"); betamin #From running this again and again I get the same results every time because there's only one possible minimum value here and the algorithm is capable of finding it every time I run it again and it will only change if the model (the Lasso regression) is changed, so the beta values below are local optima overall but global optima within this specific model in a sense 
```
betamin = coef(cv.spender, select="1se"); betamin
---
...
#Experiment
aic_value <- AIC(spender)[1]  # First value of AIC (AIC for the full model)
aicc_value <- aic_value + (2 * k * n) / (n - k - 1)
print(aicc_value)
---
