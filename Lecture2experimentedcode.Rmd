...
```{r}
  browser <- read.csv("C:/Users/aditc/OneDrive/Documents/web-browsers.csv")
  dim(browser)
  head(browser)
  tail(browser)
```
...
```{r}
  mean(browser$spend); var(browser$spend)/1e4; sqrt(var(browser$spend)/1e4); var(browser$spend); mean(browser$hispanic)
```
...
```{r, out.width = "30%", fig.align='center'}    
  ...
  #Yes, the line of code above scales the yfit normal distribution line to match the initialized histogram's scale: diff(h$mids[1:2]) gives the width of the histogram bins using midpoints (this is needed to adjust the yfit line to the scale of the histogram), yfit is needed on the right hand side because it's being scaled using * operator and length(mub) is needed to scale properly because the histogram represents frequency counts while the normal distribution represents density and length(mub) gives us the frequency counts
...
```{r}
  ...
  png(file="C:/Users/aditc/OneDriveBHAlgoExample.png",
  width=600, height=350)
  plot(pvalrank, pval, ylab="p-value", xlab="p-value rank", pch=16, col=reject)
  lines(pvalrank, (0.1/9)*pvalrank)
  dev.off()
  
  spendy <- glm(log(spend) ~ . , data=browser)
  round(summary(spendy)$coef,2)
  
  spendy <- glm(log(spend) ~ . -id, data=browser)
  round(summary(spendy)$coef,3)
```
...
pvals2 <- summary(full)$coef[-1,5] #-1 to drop the intercept, going to 5 puts us out of bounds
print(pvals)
...
hist(pvals, xlab="p-value", main="Histogram of p-values", col="lightblue")#looks like we have some 
```
...
```{r, echo=FALSE}
fdr_cut <- function(pvals, q=0.2){
  pvals <- sort(pvals[!is.na(pvals)])
  N <- length(pvals)
  k <- rank(pvals, ties.method="max")
