...
```{r}
CEO_Diary <- read.csv("C:/Users/aditc/OneDrive/Documents/survey_response_data.csv")
```
...
```{r}
CEO_Diary[1:15,c(1:5,10:12, 39, 39)] 
```
...
What does "barplot(prop.table(table(CEO_Diary$type)), las=2)" do? How can you do coding experiments to learn what this function does? - answer in my README
...
```{r}
fit <- glm(strategy ~ consultants + politicians, data=CEO_Diary, family = binomial); summary(fit)
```
