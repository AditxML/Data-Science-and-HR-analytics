...
Partial effect estimate implies 10 percent increase in price decreases sales by 31 percent. 
...
Second equation can have additional practical value when data are high dimensional $n<p$ and when you need to be selective with controls. Dimensionality means don't have enough data $n$ to estimate first equation. 
  ...
  * Can apply ML to second equation to reduce the set of control variables (to be less than $n$).
...
summary(interact)$coef[,]

```

Sign changes. No statistical significance. 
t <- factor(t)
interact <- glm(y ~ .^2, data=controls)
summary(interact)$coef[,]
---
...
In practice, can substitute residual $\hat{v} = d - \hat{d}$ for $v$:
...
treat <- cv.gamlr(x,d, lmr=1e-4); head(summary(treat))
predtreat <- predict(treat, x, select="min"); head(predtreat)
dhat <- drop(predtreat); length(dhat)

cor(drop(dhat),d)^2

treat <- cv.gamlr(x,d, lmr=1e-2); head(summary(treat))
predtreat <- predict(treat, x, select="min"); head(predtreat)
dhat <- drop(predtreat); length(dhat)

cor(drop(dhat),d)^2

treat <- cv.gamlr(x,d, lmr=1e-1); head(summary(treat))
predtreat <- predict(treat, x, select="min"); head(predtreat)
dhat <- drop(predtreat); length(dhat)
...
# Additional comments on sample splitting
  ...
  * Uncertainty quantification in second subsample ignores model selection that took place in first stage. 
    + Typical argue that this okay because model selection is "nuisance". 
