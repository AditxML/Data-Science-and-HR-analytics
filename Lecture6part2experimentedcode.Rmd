...
```{r, echo=FALSE}
library(Matrix)
data <- read.table("C:/Users/aditc/OneDrive/Documents/abortion.dat", skip=1, sep="\t")
...
y <- data$y_murd
d <- data$a_murd
cell <- read.csv("C:/Users/aditc/OneDrive/Documents/us_cellphone.csv")
...
```
# OrthoML and effect of abortion access on crime with 10 folds now

resids <- orthoLTE( x=x, d=d, y=y, 
				dreg=dreg, yreg=yreg, nfold=10) 
head(resids$dtil)
head(resids$ytil)
2*pnorm(-abs(resids$gam)/resids$se) #p-value supports no effect of abortion access on crime
---
...
descr <- read.dta("C:/Users/aditc/OneDrive/Documents/oregonhie_descriptive_vars.dta")
prgm <- read.dta("C:/Users/aditc/OneDrive/Documents/oregonhie_stateprograms_vars.dta")
s12 <- read.dta("C:/Users/aditc/OneDrive/Documents/oregonhie_survey12m_vars.dta")
...
# Digression - Handling Missings 
  ...
  * Categorical variables: 
    ...
    # Check the levels of the 'edu_12m' column again after applying `naref`
    levels(X$edu_12m)
    ```
...
mzimpute <- function(v){ 
	if(mean(v==0,na.rm=TRUE) > 0.8) impt <- 0
	else impt <- mean(v, na.rm=TRUE)
	v[is.na(v)] <- impt
	return(v) }
xnum <- apply(xnum, 2,  mzimpute)
...
tail(xnum)
xnum <- apply(xnum, 1,  mzimpute)


head(xnum)
dim(xnum)

#Please go back and revert changes to xnum before running later parts of code
...
```{r}
load("C:/Users/aditc/OneDrive/Documents/dominicks-beer.rda")
head(wber)
